...
def _call_openai(system_prompt: str, user_content: str) -> JudgeVerdict:
    ...
    for attempt in range(5):
        try:
            ...
            break
        except (openai.RateLimitError, ValueError):
            if attempt == 4:
                if 'response' in locals() and response.choices and not response.choices[0].message.content:
                    raw = ""
                    break
                raise
            time.sleep(5)
    ...
    raw = raw_content if 'raw_content' in locals() else ""
