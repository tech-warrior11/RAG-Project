...
def print_report(report: dict):
    ...
    elif f["num_evaluated"] == 0:
        print("  SKIPPED: 0 answers generated to evaluate.")
    else:
    ...
    elif c["num_evaluated"] == 0:
        print("  SKIPPED: 0 answerable-query answers generated to evaluate.")
    else:
    ...
    lat = report["latency"]
    _section("LATENCY")
    print(f"  {'stage':<16}{'p50':>9}{'p70':>9}{'p100':>9}   (ms)")
    for stage in ("embed", "search", "retrieval_total", "generation"):
        b = lat[stage]
        print(f"  {stage:<16}{b['p50_ms']:>9.2f}{b['p70_ms']:>9.2f}{b['p100_ms']:>9.2f}")
    ...
    print(f"\n  Retrieval  P100 {lat['retrieval_total']['p100_ms']:.2f}ms vs. {lat['retrieval_latency_budget_ms']}ms budget  -> {r_status}")
    print(f"  Generation P100 {lat['generation']['p100_ms']:.2f}ms vs. {lat['generation_latency_target_ms']}ms target  -> {g_status}  (suite-chosen target, see eval/checks/latency.py)")
