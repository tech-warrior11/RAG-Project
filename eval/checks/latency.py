...
DEFAULT_RETRIEVAL_LATENCY_BUDGET_MS = 50.0
...
def _block(values: list[float]) -> dict:
    ...
    return {
        ...
        "p70_ms": round(_percentile(values, 70), 2),
        "p100_ms": round(_percentile(values, 100), 2),
    }
...
def run(results: list[ExampleResult]) -> dict:
    ...
    retrieval_p100 = _percentile(retrieval_total_ms, 100)
    generation_p100 = _percentile(generation_ms, 100)
    ...
    return {
        ...
        "retrieval_within_budget": retrieval_p100 <= latency_budget_ms,
        "generation_latency_target_ms": GENERATION_LATENCY_TARGET_MS,
        "generation_within_target": generation_p100 <= GENERATION_LATENCY_TARGET_MS,
    }
